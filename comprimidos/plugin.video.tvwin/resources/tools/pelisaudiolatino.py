...
def pelis1(params):
   ...
   for entry in temps:
    ...
    if server_title.find("allmyvideos") >=0:
       ...
       plugintools.add_item(action="allmyvideoslink", title = params.get("title") + " "   +server_title , thumbnail = title_url  , url = server ,  fanart = "" , folder = False, isPlayable = True)
...
def allmyvideoslink(params):
    plugintools.log('[%s %s] allmyvideoslink %s' % (addonName, addonVersion, repr(params))) 
    url = params.get("url")
    referer = url
    data = gethttp_referer_headers(url,referer)  
    plugintools.log("data= "+data)
    final2 = plugintools.find_single_match(data, 'url=(.*?)"></iframe>')
    final2 = 'http://www.allmyvideos.net/'+final2
    plugintools.log("final2 = "+final2 )
    params = plugintools.get_params()
    params["url"]=final2
    getlink(params)
...
def getlink(params):
    ...
    if final2.find("http://played.to/") >= 0:
        ...
        params["url"]=final2
        nowvideo(params)
    elif final2.find("http://www.allmyvideos.net/") >= 0:
        params["url"]=final2
        allmyvideos(params)
