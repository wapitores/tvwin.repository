...
music = 'http://images.forwallpaper.com/files/thumbs/preview/25/251516__disco-lights_p.jpg'
...
def series(params):
   ...
   for entry in temp:
    ...
    plugintools.add_item(action="url_play12", title = server_title  , thumbnail = mini_url  , url = server_url2 ,  fanart = pel, folder = True, isPlayable = False)
...
def url_play12(params):
   ...
   for entry in temp:
    ...
    plugintools.add_item(action="play", title = url2 , thumbnail = tum , url = play32 , fanart = pel2 , folder = False , isPlayable = 	True)
...
def url_play13(params):
   ...
   for entry in temp:
    ...
    tum = plugintools.find_single_match(entry, 'thumb="([^"]+)"')
    tum = 'http://pacific1469.serverprofi24.eu:32400'+ tum + "?X-Plex-Token=yK9PAhXdNKo3ywoDx9b7"
    plugintools.add_item(action="play", title = url2 , thumbnail = tum , url = play32 , fanart = music , folder = False , isPlayable = 	True)	
