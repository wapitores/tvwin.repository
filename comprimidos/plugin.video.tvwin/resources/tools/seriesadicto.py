...
def seriecatcher(params):
    ...
    url = params.get("url")
    referer = 'http://www.yaske.to/es/peliculas/genero/premieres'
	...
	next_page = scrapertools.find_single_match(data,'href=(.*?)>&raquo;</a></div></div></div></div>')
    if next_page!="":
        plugintools.add_item(action="seriecatcher", title =">> Página siguiente" , thumbnail = "", url = next_page, fanart = fanart , folder = True)
