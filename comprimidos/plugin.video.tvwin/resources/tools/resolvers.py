...
def allmyvideos(params):
    ...
    data,response_headers = plugintools.read_body_and_headers(url_fixed, headers=request_headers)
    ...
    if "<b>File Not Found</b>" in data or "<b>Archivo no encontrado</b>" in data or '<b class="err">Deleted' in data or '<b class="err">Removed' in data or '<font class="err">No such' in data:
        ...
        # Normaliza la URL
        videoid = page_url.replace("http://allmyvideos.net/","").strip()
        ...
        for video_url in video_urls:
            plugintools.log("[allmyvideos.py] %s - %s" % (video_url[0],video_url[1]))

        plugintools.log(video_url[1])
