# -*- coding: utf-8 -*-
#------------------------------------------------------------
# Buscador Unificado de Magnets (BUM+) para PalcoTV (Kickass, Isohunt, BitSnoop, Monova)
...
def bum_multiparser(params):
    plugintools.log('[%s %s].BUM+ %s' % (addonName, addonVersion, repr(params)))    
...
def kickass0_bum(params):
    plugintools.log('[%s %s].Kickass0_bum %s' % (addonName, addonVersion, repr(params)))           
...
def kickass1_bum(params):
    plugintools.log('[%s %s].kickass1_bum %s' % (addonName, addonVersion, repr(params)))       
    ...
    for entry in matches:
        ...
        plugintools.add_item(action="play", title='[COLOR gold][I]['+seeds+'/'+leechs+'][/I][/COLOR] [COLOR white] '+match_title+' [/COLOR] [COLOR lightyellow][I]['+size + '] [Kickass][/I][/COLOR]', url=magnet_url, thumbnail = logo , fanart = fanart , show = show , extra = show , folder=False, isPlayable=False)

                            

def bitsnoop0_bum(params):
    plugintools.log('[%s %s].bitsnoop0_bum %s' % (addonName, addonVersion, repr(params)))  
...
def bitsnoop1_bum(params):
    ...
    plugintools.log('[%s %s].bitsnoop1_bum %s' % (addonName, addonVersion, repr(params)))  
    ...
    for entry in matches:
        ...
        plugintools.add_item(action="bitsnoop2_bum", title= stats+'  '+title_url+' [COLOR lightgreen][I]['+size+'] [BitSnoop][/I][/COLOR]', url = page_url , thumbnail = thumbnail, fanart = fanart, folder = False, isPlayable = False)        
        



def bitsnoop2_bum(params):
    plugintools.log('[%s %s].bitsnoop2_bum %s' % (addonName, addonVersion, repr(params))) 
...
def isohunt0_bum(params):
    plugintools.log('[%s %s].isohunt0_bum %s' % (addonName, addonVersion, repr(params))) 
...
def isohunt1_bum(params):
    plugintools.log('[%s %s].isohunt1_bum %s' % (addonName, addonVersion, repr(params)))       
    ...
    for entry in matches:
        ...
        if entry.find("Verified Torrent") >= 0:
            ...
            plugintools.add_item(action="isohunt2_bum", title= '[COLOR gold][I]['+seeds+'/'+leechs+'][/I][/COLOR] [COLOR white] '+title_url+' [/COLOR] [COLOR lightyellow][I]['+size + '] [Isohunt][/I][/COLOR]', url = page_url , thumbnail = thumbnail, fanart = fanart, folder = False, isPlayable = False)        
            
            

def isohunt2_bum(params):
    plugintools.log('[%s %s].isohunt2_bum %s' % (addonName, addonVersion, repr(params)))       
...
def monova0_bum(params):
    plugintools.log('[%s %s].Isohunt %s' % (addonName, addonVersion, repr(params)))       
...
def monova1_bum(params):
    plugintools.log('[%s %s].monova1_bum %s' % (addonName, addonVersion, repr(params)))       
    ...
    for entry in matches:
        ...
        if entry.find("Direct Download") >= 0:  # Descartamos resultados publicitarios 'Direct Download' que descargan un .exe
            ...
            plugintools.add_item(action="monova2_bum", title = semillas+'  '+title_url+' [COLOR lightgreen][I][ '+size_url+'][/I][/COLOR] ', url = page_url , thumbnail = thumbnail , fanart = fanart , folder = True, isPlayable = False)  
            
            

def monova2_bum(params):
    plugintools.log('[%s %s].Monova2_bum %s' % (addonName, addonVersion, repr(params)))       
